...
/**
 * Central registry for all {@code @HotSwap} field bindings.
 *
...
public class HotSwapRegistry {
    ...
    public HotSwapRegistry(ApplicationEventPublisher eventPublisher, TypeCoercer typeCoercer) {
    ...
    /**
     * Register a {@code @HotSwap} field binding.
    ...
    public void register(String key, FieldBinding binding) {
        ...
        if (binding.sensitive()) {
            log.info("Registered @HotSwap field: {}.{} key='{}' [SENSITIVE]",
                    binding.beanClassName(), binding.fieldName(), key);
        } else {
            log.info("Registered @HotSwap field: {}.{} key='{}' value={}",
    ...
    public void onSourceChange(String sourceId, Map<String, String> changedKeys) {
        for (var entry : changedKeys.entrySet()) {
            ...
            if (bindings == null) {
                // Key exists in config but no @HotSwap field uses it. Normal. Skip.
                continue;
            }
