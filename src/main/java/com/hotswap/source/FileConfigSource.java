 ...
 *       changed keys to {@link HotSwapRegistry#onSourceChange}</li>
 * </ul>
 *
 * @since 1.0.0
 */
public class FileConfigSource implements ConfigSource {
    ...
    private void startWatchServiceMode() {
        ...
        watchThread = new Thread(this::watchLoop, "hotswap-watcher-" + filePath.getFileName());
        watchThread.setDaemon(true);
        watchThread.start();

        // Thread 2: Safety-net CRC32 poll every 60s
        safetyNetExecutor = createScheduledExecutor("hotswap-safety-net-" + filePath.getFileName());
    ...
    private void startDegradedPollMode() {
        safetyNetExecutor = createScheduledExecutor("hotswap-degraded-poll-" + filePath.getFileName());
        safetyNetExecutor.scheduleAtFixedRate(
                this::safetyNetPoll,
