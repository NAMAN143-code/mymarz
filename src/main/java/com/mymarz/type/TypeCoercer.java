 ...
 *
 * <p>Handles both explicit {@link MarzType} hints and automatic
...
public class TypeCoercer {
     ...
     * Used by {@link com.mymarz.core.MarzRegistry#onSourceChange} where
     ...
     * @throws MarzTypeException if coercion fails
     */
    public Object coerce(String rawValue, Class<?> targetType) {
        ...
        } catch (MarzTypeException e) {
            throw e;
        } catch (Exception e) {
            throw new MarzTypeException(
     ...
     * @throws MarzTypeException if coercion fails
     */
    public Object coerce(String rawValue, MarzType type, Field field) {
        if (rawValue == null) {
            return null;
        }

        MarzType resolvedType = type;
        if (type == MarzType.INFERRED) {
    ...
    /**
     * Infer the {@link MarzType} from a field's declared Java type.
     *
     * @param field the annotated field
     * @return the inferred MarzType
     */
    public MarzType inferType(Field field) {
        Class<?> fieldType = field.getType();

        if (fieldType == boolean.class || fieldType == Boolean.class) {
            return MarzType.BOOLEAN;
        }
        if (fieldType == String.class) {
            return MarzType.STRING;
        }
        if (fieldType == int.class || fieldType == Integer.class) {
            return MarzType.INTEGER;
        }
        if (fieldType == long.class || fieldType == Long.class) {
            return MarzType.LONG;
        }
        if (fieldType == double.class || fieldType == Double.class) {
            return MarzType.DOUBLE;
        }
        ...
        return MarzType.JSON;
    }

    private Object doCoerce(String rawValue, MarzType type, Field field) throws Exception {
        return switch (type) {
    ...
    private Boolean parseBoolean(String value) {
        ...
        throw new MarzTypeException("Cannot parse boolean from: '" + value + "'");
    }
}
